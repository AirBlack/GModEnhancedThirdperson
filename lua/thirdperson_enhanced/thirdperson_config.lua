if CLIENT then
	...
	hook.Add("PopulateToolMenu", "ThirdpersonSandboxMenu", function()
		spawnmenu.AddToolMenuOption("Utilities", "User", "thirdperson_etp_options", "Thirdperson", "", "", function(panel)
			...
			local function createCameraCollisionComboBox()
				local combobox, _ = panel:ComboBox("Camera collision", "thirdperson_etp_cameracollisionmode")

				combobox:AddChoice("Everything", THIRDPERSON_ENHANCED.CameraCollisionMode.worldPropsPlayersAndNpcs)
				combobox:AddChoice("Ignore players and NPCs", THIRDPERSON_ENHANCED.CameraCollisionMode.worldAndprops)
				combobox:AddChoice("World only", THIRDPERSON_ENHANCED.CameraCollisionMode.worldOnly)

				panel:ControlHelp("Not saved in presets")
			end

			createCameraCollisionComboBox()
