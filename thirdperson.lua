...
if CLIENT then
	...
	concommand.Add("thirdperson_enhanced_toggle", function(ply, cmd, args)
		...
		if GetConVar("thirdperson_etp_addons_sync"):GetBool() then
			if thirdPerson:GetBool() then
				if GetConVar("gtvh_firstperson"):GetInt() == 1 then ply:ConCommand("gtvh_firstperson 0") end
			else
				if GetConVar("gtvh_firstperson"):GetInt() == 0 then ply:ConCommand("gtvh_firstperson 1") end
			end
		end
	end)
	...
	hook.Add("CalcView", "thirdperson_etp", function(ply, pos, ang, fov, znear, zfar)
		...
		local fov = GetConVar("thirdperson_etp_fov"):GetFloat()
		...
		if ply:Crouching() and not useheadPos then offsetZ = offsetZ + crouchadd end
	...
	hook.Add("ShouldDrawLocalPlayer", "EnhancedThirdpersonShouldDrawLocalPlayer", function(fply)
		if not GetConVar("thirdperson_etp"):GetBool() then return end
		return true
	end)
...
elseif SERVER then
	...
	hook.Add("PlayerEnteredVehicle", "EnhancedThirdpersonСheckVehicleEntered", function(ply, veh, role)
		...
		if IsValid(veh) and thirdpersonenabled then
			veh:SetThirdPersonMode(true)
		else
			veh:SetThirdPersonMode(false)
		end
